...
* 🔥 плюсы - ветка не завалена коммитами + rebase-коммиты читабельны 
* 🔥 линейность истории
* базовое использование:
> на ветке `feature` совершаем действия
> переключаюсь на `master` и там совершаем действия
> переключаюсь на `feature` и `git rebase master` - коммит ветки `feature` будет помещён "сверху"(визуально выглядит как слияние)
### 💥 Делаю конфликт (возникает на ветках когда в одних и тех же рядках разные данные )
* в файле `all.txt` на одной и той же строке вношу разные данные в ветках вначале `featers` затем `master`
* если сейчас переключиться на `featers` и `git rebase master` то будет конфликт
* давай выберем из опций решений ошибки `accept incoming change` = в `all.txt` будет текст из данных на `featers`
* `git add .` и `git rebase --continue` = будет коммит с изменеиями 

### `git rebase -i HEAD~5`
> это интерактивный `rebase` обычно используют перед отправкой в главную ветку для очистки нашей ветки
> `HEAD~5` берём последние 5 коммитов
> выбираем команды в редакторе
